          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <div style={{ padding: 24 }}>
                ...
                <span style={{ backgroundColor: tone.primary, color: "#fff", fontSize: 12, fontWeight: 600, padding: "4px 12px", borderRadius: 999, display: "inline-block" }}>
                  {item.tag}
                </span>
              </div>
            </div>
          ...
          {items.map((item, i) => (
            <div key={i}>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 py-6">
                <div className="flex-shrink-0" style={{ width: 80, height: 80, borderRadius: tone.radius, overflow: "hidden" }}>
                  {item.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={item.image} alt="" style={{ width: 80, height: 80, objectFit: "cover", display: "block" }} />
                  ) : (
                    <div style={{ width: 80, height: 80, background: tone.gradient, borderRadius: tone.radius }} />
                  )}
                </div>
                <div className="flex-1">
                  <h3 style={{ fontFamily: tone.headingFont, fontWeight: 700, fontSize: 18, color: tone.text, marginBottom: 4 }}>
                    {item.title}
                  </h3>
                  <p style={{ color: tone.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 8 }}>
                    {item.description}
                  </p>
                  <span style={{ backgroundColor: tone.primary, color: "#fff", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 999, display: "inline-block" }}>
                    {item.tag}
                  </span>
                </div>
              </div>
  ...
  return (
    <section className="py-20 px-6" style={{ backgroundColor: tone.surface, fontFamily: tone.bodyFont }}>
      <div className="max-w-6xl mx-auto">
        ...
        {featured && (
          <div style={{ position: "relative", borderRadius: tone.radius, overflow: "hidden", marginBottom: 32 }}>
            <CardImage src={featured.image} gradient={tone.gradient} height={320} />
            <div
              style={{
                position: "absolute",
                bottom: 0,
            ...
            >
              <span style={{ backgroundColor: tone.primary, color: "#fff", fontSize: 12, fontWeight: 600, padding: "4px 12px", borderRadius: 999, display: "inline-block", marginBottom: 10 }}>
                {featured.tag}
              </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {rest.map((item, i) => (
            <div key={i} style={{ backgroundColor: tone.bg, borderRadius: tone.radius, overflow: "hidden", border: `1px solid ${tone.border}` }}>
              <CardImage src={item.image} gradient={tone.gradient} height={140} />
              <div style={{ padding: 20 }}>
                <span style={{ backgroundColor: tone.primary, color: "#fff", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 999, display: "inline-block", marginBottom: 10 }}>
                  {item.tag}
                </span>

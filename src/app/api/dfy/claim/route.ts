...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("DFY claim insert error:", error);
...
Make each article 2-3 paragraphs, informative and helpful for ${niche} audience.`;
    ...
    const response = await fetch(`https://${apiHost}/conversationgpt4`, {
      ...
      body: JSON.stringify({
        ...
        system_prompt:
          "You are a professional content writer. Return ONLY valid JSON array, no markdown.",
        temperature: 0.7,
        max_tokens: 3000,
        web_access: false,
      }),
    ...
    const text =
      result?.result || result?.response || result?.choices?.[0]?.message?.content || "";
